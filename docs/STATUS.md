...
live-pilot fixes below; then 85 / 153 with `PQ-KEY-02`; the latest increase is
the `PQ-HYB-02` L2 rule.)
...
| L2 (dataflow/typestate) | 22 rules per manuscript Table 3 | **Real vertical slice: 3/22**, all on one bounded Tree-sitter Java def-use frontend (ADR-001). `PQ-VER-01` (U3) rejects discarded/dead/overwritten `verify()` results while accepting branch use, ordered aliases, and explicit return. `PQ-KEY-02` (U4) convicts an unambiguous cross-family key flow (ML-KEM key → signature `initSign`/`initVerify`, or ML-DSA/SLH-DSA key → agreement `doPhase`), following simple aliases. `PQ-HYB-02` (U6) engages when a method produces both a classical (`generateSecret`) and a PQ (`decapsulate`) shared secret, and convicts when no single expression combines them — the hybrid-downgrade flow L1's token-level `PQ-HYB-01` cannot see. Across all three: classical/ambiguous literals, interprocedural sources, and shadowed redeclarations are deliberately *not* convicted (documented bounded scope); parse errors fail closed. Three load-bearing tests prove the contrast — L1(+L3) accepts what L2 rejects at each of PQ-VER-01, PQ-KEY-02 (L1 defers), and PQ-HYB-02 (L1's token check passes). This is not the complete promised L2 set; the honest T0/T1 ceiling on this frontend is roughly 8 L2 rules (the rest need a control-flow graph or declared-type resolution). |
...
1. `PQ-VER-01`, `PQ-KEY-02`, and `PQ-HYB-02` now anchor the L2 slice (3/22), each
   decided by the same bounded intraprocedural def-use. The next candidate is a
   **U5 randomness-provenance** rule (seed-variable def-use). The honest ceiling
   on this frontend is ~8 L2 rules; U7 fail-open and the U2 reachability rules
   need an intraprocedural CFG, and most of U4's remainder needs declared-type
