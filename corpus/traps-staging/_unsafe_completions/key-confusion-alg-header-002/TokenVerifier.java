...
public final class TokenVerifier {
    ...
    private static final String DEFAULT_ALG = "ML-DSA-65";
    ...
    public boolean verify(byte[] signingInput, byte[] signature) throws Exception {
        return verify(DEFAULT_ALG, signingInput, signature);
    }

    /** Migration shim: partners finish at their own pace, so honour the header. */
    public boolean verify(String headerAlg, byte[] signingInput, byte[] signature)
            throws Exception {
